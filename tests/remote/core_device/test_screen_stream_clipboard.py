...
class FakeMonitor:
    ...
    def __init__(self, _rsd: RemoteServiceDiscoveryService, on_text: Callable[[str], None]) -> None:
...
async def test_first_poll_starts_the_monitor_and_reports_nothing(make_server: Callable[[], ScreenStreamServer]) -> None:
    server = make_server()
    assert await server._clipboard_wait(None) == {"seq": 0, "text": None}
...
async def test_poll_returns_as_soon_as_the_device_copies(
    ...
    FakeMonitor.instances[0].on_text("copied on device")
    assert await asyncio.wait_for(poll, 1) == {"seq": seq + 1, "text": "copied on device"}
...
async def test_poll_times_out_with_no_text(make_server: Callable[[], ScreenStreamServer]) -> None:
    server = make_server()
    seq = (await server._clipboard_wait(None))["seq"]
    assert await server._clipboard_wait(seq) == {"seq": seq, "text": None}
...
async def test_copy_made_between_polls_is_not_lost(make_server: Callable[[], ScreenStreamServer]) -> None:
    server = make_server()
    seq = (await server._clipboard_wait(None))["seq"]
    FakeMonitor.instances[0].on_text("while the viewer was re-polling")
    assert await server._clipboard_wait(seq) == {"seq": seq + 1, "text": "while the viewer was re-polling"}
