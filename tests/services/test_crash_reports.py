...
def test_ls_default(crash_manager: CrashReportsManager, delete_test_dir: None) -> None:
    crash_manager.afc.makedirs(PATH_COMPONENT)
    assert PATH_COMPONENT in crash_manager.ls()


def test_ls_path(crash_manager: CrashReportsManager, delete_test_dir: None) -> None:
    crash_manager.afc.makedirs(PATH_COMPONENT * 2)
    assert (PATH_COMPONENT * 2) in crash_manager.ls(path=PATH_COMPONENT)


@pytest.mark.parametrize("depth", [2, 3, 4])
def test_ls_depth(crash_manager: CrashReportsManager, delete_test_dir: None, depth: int) -> None:
...
def test_ls_depth_minus_one(crash_manager: CrashReportsManager, delete_test_dir: None) -> None:
...
def test_pull(crash_manager, delete_test_dir: None) -> None:
    crash_manager.afc.makedirs(PATH_COMPONENT)
