...
from construct import Struct, Const, Int64ul, Container
...
class AFCClient(object):
    ...
    def do_operation(self, opcode, data: bytes = b""):
    ...
    def read_directory(self, dirname):
        status, data = self.do_operation(AFC_OP_READ_DIR, dirname.encode())
        if status == AFC_E_SUCCESS:
    ...
    def make_directory(self, dirname):
        status, data = self.do_operation(AFC_OP_MAKE_DIR, dirname.encode())
        return status
    ...
    def get_file_info(self, filename):
        status, data = self.do_operation(AFC_OP_GET_FILE_INFO, filename.encode())
        if status == AFC_E_SUCCESS:
    ...
    def make_link(self, target, linkname, type=AFC_SYMLINK):
        ...
        separator = b"\x00"
        status, data = self.do_operation(AFC_OP_MAKE_LINK,
    ...
    def file_read(self, handle, sz):
        ...
        while sz > 0:
            ...
            try:
                ...
                status, chunk = self.receive_data()
            except:
                import traceback
                traceback.print_exc()
    ...
    def set_file_contents(self, filename, data):
        ...
        if not h:
            raise IOError('failed to create remote file')
...
class AFCShell(Cmd):
    def __init__(self, afcname='com.apple.afc', completekey='tab', stdin=None, stdout=None, client=None, udid=None,
        ...
        self.logger = logger or logging.getLogger(__name__)
        self.lockdown = LockdownClient()
        self.afc = client if client else AFCClient(self.lockdown, service_name=afcname, udid=udid)
        self.curdir = '/'
    ...
    def do_push(self, p):
        ...
        if os.path.isdir(src):
            ...
            if not src.startswith("."):
                ...
                self.afc.set_file_contents(self.curdir + "/" + dst, data)
