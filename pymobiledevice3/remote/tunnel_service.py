...
class OSRemotePairingTunnel(RemotePairingTunnel):
    ...
    async def start_tunnel(self, address: str, netmask: str, mtu: int) -> None:
        ...
        # TODO find out why netmask cannot be assigned
...
class UserspaceRemotePairingTunnel(RemotePairingTunnel):
    def __init__(self):
        ...
        self._intf = None
        self._userspace_host = "0.0.0.0"

    def set_userspace_host(self, host: str) -> None:
        self._userspace_host = host
    ...
    def _run_coroutine(self, coro: CoroutineType) -> None:
    ...
    async def start_userspace_tun(self) -> None:
        # TODO: listen addr
        self._userspace_tun_server = await asyncio.start_server(self.client_handler, self._userspace_host, 0)
    ...
    async def stop_tunnel(self) -> None:
        ...
        await asyncio.gather(self._intf_poll_task, *self._pending_tasks, return_exceptions=True)
        self._intf = None
...
class StartTcpTunnel(ABC):
    ...
    @abstractmethod
    async def start_tcp_tunnel(self, userspace_host: Optional[str]) -> AsyncGenerator[TunnelResult, None]:
        pass
...
class RemotePairingProtocol(StartTcpTunnel):
    ...
    async def start_quic_tunnel(
            ...
            userspace_host: Optional[str] = None) -> AsyncGenerator[TunnelResult, None]:
        ...
        try:
            async with aioquic_connect(
                    ...
                    create_protocol=UserspaceRemotePairingQuicTunnel if use_userspace else OSRemotePairingQuicTunnel,
            ) as client:
                self.logger.debug('quic connected')
                # Since Userspace*Tunnel has .set_userspace_host extra param, we just cast it as Userspace
                # but client is Userspace*Tunnel if use_userspace else OS*Tunnel
                client = cast(UserspaceRemotePairingQuicTunnel, client)
                if use_userspace:
                    client.set_userspace_host(userspace_host)
    ...
    async def start_tcp_tunnel(self, userspace_host: Optional[str]) -> AsyncGenerator[TunnelResult, None]:
        ...
        use_userspace = userspace_host is not None
        tunnel = (UserspaceRemotePairingTcpTunnel if use_userspace else OSRemotePairingTcpTunnel)(reader, writer)
        if use_userspace:
            tunnel.set_userspace_host(userspace_host)
...
class CoreDeviceTunnelProxy(StartTcpTunnel):
    ...
    @asynccontextmanager
    async def start_tcp_tunnel(self, userspace_host: Optional[str] = None) -> AsyncGenerator['TunnelResult', None]:
...
async def start_tunnel_over_remotepairing(
        ...
        max_idle_timeout: float = _RemotePairingQuicTunnelMixin.MAX_IDLE_TIMEOUT,
        protocol: TunnelProtocol = TunnelProtocol.QUIC, userspace_host: Optional[str] = None) \
    ...
    async with remote_pairing:
        if protocol == TunnelProtocol.QUIC:
            async with remote_pairing.start_quic_tunnel(
                    ...
                    userspace_host=userspace_host) as tunnel_result:
                yield tunnel_result
        elif protocol == TunnelProtocol.TCP:
            async with remote_pairing.start_tcp_tunnel(userspace_host=userspace_host) as tunnel_result:
                yield tunnel_result
...
async def start_tunnel_over_core_device(
    ...
    async with service_provider:
        if protocol == TunnelProtocol.QUIC:
            ...
            async with service_provider.start_tcp_tunnel(userspace_host=userspace_host) as tunnel_result:
                resume_remoted_if_required()
                yield tunnel_result
...
async def start_tunnel(
    ...
    if isinstance(protocol_handler, CoreDeviceTunnelService):
        async with start_tunnel_over_core_device(
                ...
                protocol=protocol, userspace_host=userspace_host) as service:
            yield service
        ...
        async with protocol_handler.start_tcp_tunnel(userspace_host=userspace_host) as service:
            yield service
    else:
