    ...
    async def sock_read_task(self) -> None:
        try:
            while True:
                ...
                except (asyncio.exceptions.IncompleteReadError, ConnectionAbortedError):
                    return
        except OSError as e:
        ...
        for address in answer.addresses:
            for identifier in iter_remote_paired_identifiers():
                ...
                try:
                    ...
                    result.append(conn)
                    break
                except (ConnectionAbortedError, asyncio.IncompleteReadError, ConnectionResetError) as e:
                    if conn is not None:
                        await conn.close()
                    logger.debug(
