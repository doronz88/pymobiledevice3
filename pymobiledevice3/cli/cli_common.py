...
from pymobiledevice3.utils import run_async_compat
...
def async_command(func: Callable[P, Awaitable[R]]) -> Callable[P, R]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        return run_async_compat(func(*args, **kwargs))
...
def make_rsd_dependency(*, allow_none: bool) -> Callable[..., Optional[RemoteServiceDiscoveryService]]:
    def rsd_dependency(
        ...
        if rsd is not None:
            ...
            run_async_compat(rsd_service.connect())
            return rsd_service

        if tunnel is not None or not allow_none:
            return run_async_compat(_tunneld(tunnel or ""))
...
def any_service_provider_dependency(
    ...
    if mobdev2:
        devices = run_async_compat(get_mobdev2_devices(udid=udid))
        if not devices:
            raise NoDeviceConnectedError()
    ...
    if udid is not None:
        return run_async_compat(create_using_usbmux(serial=udid, usbmux_address=usbmux))

    devices = run_async_compat(usbmuxd.select_devices_by_connection_type(connection_type="USB", usbmux_address=usbmux))
    if len(devices) <= 1:
        return run_async_compat(create_using_usbmux(usbmux_address=usbmux))

    lockdownds = [
        run_async_compat(create_using_usbmux(serial=device.serial, usbmux_address=usbmux)) for device in devices
    ]
...
def no_autopair_service_provider_dependency(
    ...
    return run_async_compat(create_using_usbmux(serial=udid, autopair=False))
