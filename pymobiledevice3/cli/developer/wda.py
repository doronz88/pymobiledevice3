...
async def wait_for_xctest_app(service_provider: LockdownServiceProvider, xctrunner: str) -> asyncio.Future:
    ...
    cfg = TestConfig.create_for(service_provider, xctrunner)

    ts = XCUITestService(service_provider)
    xctrunner = asyncio.ensure_future(ts.run(cfg))
...
async def wda_list_items(
    ...
    clickable: Annotated[
        ...
        typer.Option(
            "--clickable",
            help="Filter by a clickable type allowlist + visible/enabled.",
        ),
    ] = False,
    types: Annotated[
        Optional[list[str]],
        typer.Option(
            "--types",
            help="Comma-separated element type allowlist (e.g. Icon,Button,Cell).",
        ),
    ] = None,
    with_rect: Annotated[
        bool,
    ...
    if types:
        ...
        for entry in types:
            for item in entry.split(","):
                ...
                if not item:
                    continue
                if item.startswith("XCUIElementType"):
                    custom.add(item)
                else:
                    custom.add(f"XCUIElementType{item}")
        if custom:
    ...
    for elem in root.iter():
        ...
        if hittable_only and hittable != "true":
            continue
        if clickable:
            if visible != "true" or enabled != "true":
                continue
