...
class ValeriaLibusb(IOSScreenCapture):
    ...
    def start(self) -> None:
        ...
        if dev is None:
            if self._udid:
                logger.error('Valeria: no Apple device found with UDID %s', self._udid)
            else:
                logger.error('Valeria: no Apple device found over USB')
            raise DeviceNotFoundError(udid=self._udid or '')
