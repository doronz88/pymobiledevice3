...
class Pmd3TyperGroup(TyperGroup):
    ...
    def highlight_keyword(text: str, keyword: str) -> str:
        ...
        return re.sub(f'({re.escape(keyword)})', lambda m: typer.style(m.group(1), bold=True), text, flags=re.IGNORECASE)
...
def invoke_cli_with_error_handling() -> bool:
    ...
    try:
        ...
        elif (getattr(e, "identifier", None) is not None) and ('developer' in sys.argv) and ('--tunnel' not in sys.argv):
